...
<template>
	<div class="">
		...
		<div class="headImgBox"
			:style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':'url(static/img/libcdut2.jpeg)'}">
			<!-- <div class="scene">
			<div><span id="luke"></span></div>
		</div> -->
			<div class="h-information">

				<img :src="this.$store.state.themeObj.head_portrait?this.$store.state.themeObj.head_portrait:'static/img/xuruihang.jpeg'"
					alt="">
...
<script>
	...
	export default {
		...
		methods: { //事件处理器
			...
			logoinFun: function (msg) { //1：用户登录；0：注册跳转
				// console.log(msg);
		...
		created() { //生命周期函数
			//判断当前页面是否被隐藏
			var that = this;
			//根据浏览器的支持情况，hiddenProperty的值将是hidden、webkitHidden、mozHidden其中之一，或者为null表示不支持隐藏属性。
			// 在后续的代码中，根据hiddenProperty的值，可以判断页面是否被隐藏，从而执行相应的操作。
			var hiddenProperty = 'hidden' in document ? 'hidden' :
				'webkitHidden' in document ? 'webkitHidden' :
					'mozHidden' in document ? 'mozHidden' :
						null;
			//此自定义事件名是为了兼容不同浏览器，确保代码在不同浏览器中都能正确地监听和处理可见性改变事件
